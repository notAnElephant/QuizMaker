    ...
    <div className="flex flex-col items-center justify-center">
      <div className="max-w-screen-xl w-full overflow-x-auto px-4">
        <div
          ...
          style={{
            gridTemplateColumns: `repeat(${data.length}, minmax(100px, 1fr))`,
          }}
        >
          {data.map((cat, colIndex) => {
            ...
            return (
              <div key={colIndex} className="flex flex-col items-center gap-2">
                <h2 className={cn("font-bold text-2xl text-center", font)}>
                  {cat.category}
                </h2>
                {cat.questions.map((q, rowIndex) => {
                  const isDisabled =
                    q.isUsed ||
                    (classicMode && rowIndex !== firstUnansweredIndex);
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setTimeout(
                          () =>
                            navigate(
                              `/question/${colIndex + 1}/${rowIndex + 1}`,
                            ),
                          0,
                        );
                      }}
